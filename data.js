...
document.addEventListener('DOMContentLoaded', initApp);            <div class="being-tile" data-id="${b.id}" draggable="true" onclick="openInspectModal('${b.id}')" style="--strata-color: ${strataColors[b.cat] || '#6BCB77'}">
                <div class="tile-icon">${b.icon}</div>
                <div class="tile-name">${b.name}</div>
            </div>`).join('');
    }
